...
def log_parser(
    ...
    rest=None,
    tag="epoch_val_auc",
):  # type: (IOBase, int, int, int, str, [str], str) -> (str, [(float, float)])
    ...
    def process_dir(_directory):
        for fname in listdir(_directory):
            ...
            if path.isfile(full_path) and full_path.rpartition(path.extsep)[
                2
            ] not in frozenset(("h5", "tf", "dot", "profile-empty", "trace")):
    ...
    for fname in files:
        ...
        try:
            total_images += sum(
                1 for _ in tf.data.TFRecordDataset(fname)
            )  # Check corrupted tf records
        except:
        ...
        if tag == "all":
            ...
            for record in tf.data.TFRecordDataset(fname):
                ...
                if event.HasField("summary"):
                    ...
                    stdout and print(
                        "{idx:04d}\t{simple_value:09f}\t{tag:>20}\t{dirn}".format(
                            **last_result
                        )
                    )
            ...
            sorted_values = tuple(
                sorted(enumerate(values), key=itemgetter(1), reverse=True)
            )

            stdout and print(
                "\n".join(
                    "{dirn}\tmodel-{k:04d}{ext}\t{v}".format(
                        dirn=dirn.ljust(54), k=k, v=v, ext=SAVE_FORMAT_WITH_SEP
                    )
