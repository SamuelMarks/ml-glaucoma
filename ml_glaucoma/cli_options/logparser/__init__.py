...
class ConfigurableLogParser(Configurable):
    ...
    def fill_self(self, parser):
        ...
        parser.add_argument(
            "-d", "--directory", help="Directory. Searches here rather than infile."
        )
        parser.add_argument(
            "--threshold",
            help="E.g.: 0.7 for sensitivity & specificity >= 70%%",
            default="0.7",
            type=float,
        )
        ...
        parser.add_argument(
            "--by-diff",
            help="Sort by lowest difference between sensitivity & specificity",
            action="store_true",
        )
