...
def dr_spoc_builder(
    ...
    manual_dir,
    force_create=False,
):  # type: (str,str,str,str,str,bool) -> (((int, bool, str) -> (tfds.image.ImageLabelFolder)), str, str)
    assert dataset_name in dr_spoc_datasets_set, "{!r} not found in {!r}".format(
        dataset_name, dr_spoc_datasets_set
    )
    ...
    if dr_spoc_init:
        ...
        if manual_dir is None:
            raise ValueError("`manual_dir` must be provided if `dr_spoc_init is True`")
        elif dr_spoc_parent_dir is None:
            raise ValueError(
                "`dr_spoc_parent_dir` must be provided if " "`dr_spoc_init is True`"
            )
        elif force_create or not path.isdir(
            path.join(_get_manual_dir(dr_spoc_parent_dir, manual_dir), dataset_name)
        ):
    ...
    def builder_factory(
    ...
    ):  # type: (int, bool, str) -> tfds.image.ImageLabelFolder
        ...
        builder = DrSpocImageLabelFolder(dataset_name=dataset_name, data_dir=data_dir)
    ...
    return (
        builder_factory,
        data_dir,
        manual_dir if manual_dir is None else os.path.join(manual_dir, dataset_name),
    )
...
def _get_manual_dir(dr_spoc_parent_dir, manual_dir):  # type: (str, str) -> str
    return (
        path.join(dr_spoc_parent_dir, "symlinked_datasets")
        if all(
            (
                path.dirname(manual_dir) != "DR SPOC Dataset",
