...
from ml_glaucoma import get_logger, runners
from ml_glaucoma.cli_options.logparser.utils import parse_line
from ml_glaucoma.runners.utils import batch_steps, default_model_dir
...
def train(
    ...
    with strategy_scope():
        ...
        model.compile(optimizer=optimizer, loss=problem.loss, metrics=problem.metrics)

    train_steps = batch_steps(problem.examples_per_epoch("train"), batch_size)
    validation_steps = batch_steps(problem.examples_per_epoch("validation"), batch_size)
    ...
    try:
        ...
        if dot is not None:
            dotfile = os.path.join(
                os.path.dirname(model_dir), os.path.basename(model_dir) + ".dot"
            )
            dot.write(dotfile)
    ...
    print(
        ...
        "\n",
        "transfer:".ljust(just),
        parsed_line.transfer,
        "\n",
        "optimizer:".ljust(just),
        optimizer.__class__.__name__,
        "\n",
        "loss:".ljust(just),
        problem.loss.__class__.__name__,
        "\n",
        "callbacks:".ljust(just),
        ", ".join(map(lambda c: type(c).__name__, callbacks)),
        "\n",
        "metrics:".ljust(just),
        ", ".join(map(lambda m: type(m).__name__, problem.metrics)),
        "\n",
        "total_epochs:".ljust(just),
        epochs,
        "\n",
        "_" * 65,
        "\n",
    ...
    for inputs, labels in train_ds.take(1):
        model.loss(labels, model(inputs))
        MobileNet
...
def evaluate(problem, batch_size, model_fn, optimizer, model_dir=None):
    ...
    manager_cb = cb.LoadingModelCheckpoint(model_dir=model_dir, save_freq="epoch")
