    ...
    def confusion_logger(self):
        with tf.compat.v1.Graph().as_default():
            ...
            with self.writer.as_default(), tf.name_scope("auc"):
                tf.summary.scalar(
                    "tp", data=self.true_positives, description="true_positives"
                )
                tf.summary.scalar(
                    "fp", data=self.false_positives, description="false_positives"
                )
                tf.summary.scalar(
                    "tn", data=self.true_negatives, description="true_negatives"
                )
                tf.summary.scalar(
                    "fn", data=self.false_negatives, description="false_negatives"
                )
