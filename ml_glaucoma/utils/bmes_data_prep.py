...
from ml_glaucoma.utils import it_consumes, pp, redis_cursor, run_once
...
rand_cache_obj = Cache(
    fname=path.join(
        path.dirname(path.dirname(__file__)), "_data", ".cache", "rand_cache.pkl"
    )
)
...
def get_data(
    ...
    pickled_cache = (
        ...
        if path.getsize("generated_types.py") > 50
        else {}
    )
...
def old(no_oags, oags, skip_save):
    ...
    logger.debug(
        "len(id_to_img_dims):".ljust(just) + "{:d}".format(len(id_to_img_dims))
    )
    logger.debug(
        "len(img_dims_to_recimg):".ljust(just) + "{:d}".format(len(img_dims_to_recimg))
    )
    logger.debug(
        "img_dims_to_recimg.keys():".ljust(just)
        + "{}".format(img_dims_to_recimg.keys())
    )
    logger.debug("img_dims_to_recimg:".ljust(just) + "{}".format(img_dims_to_recimg))
    logger.debug(
        "# without oags1 but with loag1 etc.:".ljust(just)
        + "{:d}".format(
            sum(
                1
                for IDNUM, u in iteritems(tbl)
                if not u.rec.oag1 and (u.rec.roag1 or u.rec.loag1)
            )
        )
    )
    logger.debug(
        "# with loag1:".ljust(just)
        + "{:d}".format(sum(1 for IDNUM, u in iteritems(tbl) if u.rec.loag1))
    )
    logger.debug(
        "# with roag1:".ljust(just)
        + "{:d}".format(sum(1 for IDNUM, u in iteritems(tbl) if u.rec.roag1))
    )
    for dim in img_dims_to_recimg:
        logger.debug(
            "len(img_dims_to_recimg[{:d}]):".format(dim).ljust(just)
            + "{:d}".format(len(img_dims_to_recimg[dim]))
        )
        logger.debug(
            "{:d} with oags1:".format(dim).ljust(just)
            + "{:d}".format(
